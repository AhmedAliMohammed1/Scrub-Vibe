...
export default async function AccountPage({ params, searchParams }: Props) {
  ...
  const canAdmin = canAccessAdminDashboard(
        ...
        <aside className="self-start rounded-xs border border-[var(--border-subtle)] bg-[var(--surface-raised)] p-6 shadow-subtle md:p-8 backdrop-blur-sm">
          ...
          <dl className="mt-6 divide-y divide-[var(--border-subtle)] text-sm">
            <div className="py-4">
              ...
              <dd className="mt-2 break-all text-[var(--text-primary)] font-mono text-xs">
                {email}
              </dd>
            </div>
            <div className="py-4">
          ...
          {canAdmin && (
            ...
            >
              {locale === "ar" ? "لوحة الإدارة والتحكم" : "Admin Dashboard"}
            </Link>
          )}
