...
const statuses: TrackedOrder["status"][] = [
  "awaiting_payment",
  "payment_review",
  "confirmed",
  "processing",
...
const paymentStatuses: TrackedOrder["payment_status"][] = [
  "pending",
  "proof_submitted",
  "paid",
  "partially_refunded",
...
export default async function AdminOrdersPage({
  ...
  const [ordersResult, webhookResult, recoveryStats] = await Promise.all([
    ...
    admin
      ...
      .select(
        "id, provider_event_id, transaction_id, outcome, error_code, received_at",
      )
  ...
  await Promise.all(
    orders.flatMap((order) =>
      order.payment_proofs.map(async (proof) => {
  ...
  const paidOrders = orders.filter((order) =>
    ["paid", "cod_collected"].includes(order.payment_status),
  );
  const revenue = paidOrders.reduce((sum, order) => sum + order.total_minor, 0);
  const awaitingReview = orders.filter(
    (order) => order.payment_status === "proof_submitted",
  ).length;
  const fulfilmentQueue = orders.filter((order) =>
  ...
  return (
    <main className="min-h-screen bg-[#eef2ef]">
      <header className="bg-[#062f2b] text-white">
        <div className="mx-auto max-w-[1500px] px-5 py-10 md:px-10">
          <div className="flex flex-wrap items-end justify-between gap-5">
            <div>
              <p className="eyebrow text-[#81c5b8]">SCRUB VIBE · OPERATIONS</p>
              <h1 className="mt-3 font-serif text-5xl md:text-7xl">
                {ar ? "الطلبات والمدفوعات" : "Orders & payments"}
              </h1>
            </div>
      ...
      <div className="mx-auto max-w-[1500px] px-5 py-8 md:px-10 md:py-12">
        ...
        <section className="mt-5 border border-[#0e7468]/30 bg-white p-5 md:p-6">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-black/10 pb-4">
            ...
            <span className="bg-[#dce9e5] px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[#073b36]">
              {ar
                ? `نسبة الاستعادة: ${recoveryStats.recoveryRatePercent}٪`
                : `Recovery rate: ${recoveryStats.recoveryRatePercent}%`}
            </span>
          </div>
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div className="border border-black/5 bg-[#f4f7f4] p-4">
              <p className="text-[10px] font-bold uppercase text-neutral-500">
                {ar ? "تذكيرات مرسلة" : "Reminders sent"}
              </p>
              <strong className="mt-1 block font-serif text-2xl text-[#062f2b]">
                {recoveryStats.totalSent}
              </strong>
            </div>
            <div className="border border-black/5 bg-[#f4f7f4] p-4">
              <p className="text-[10px] font-bold uppercase text-neutral-500">
                {ar ? "سلات مستعادة" : "Carts recovered"}
              </p>
              <strong className="mt-1 block font-serif text-2xl text-[#0e7468]">
                {recoveryStats.totalRecovered}
              </strong>
            </div>
            <div className="border border-black/5 bg-[#f4f7f4] p-4">
              <p className="text-[10px] font-bold uppercase text-neutral-500">
                {ar ? "إيراد مسترد" : "Recovered revenue"}
              </p>
              <strong className="mt-1 block font-serif text-2xl text-[#062f2b]">
                {formatMoney(recoveryStats.recoveredRevenueMinor, locale)}
              </strong>
            </div>
            <div className="border border-black/5 bg-[#f4f7f4] p-4">
              <p className="text-[10px] font-bold uppercase text-neutral-500">
                {ar ? "المراحل" : "Stage breakdown"}
              </p>
              <p className="mt-1 text-[11px] leading-5 text-neutral-600">
                1st (2h): {recoveryStats.byStage.firstReminder.sent} · 2nd
                (24h): {recoveryStats.byStage.secondReminder.sent} · 3rd (48h):{" "}
                {recoveryStats.byStage.discountOffer.sent}
              </p>
            </div>
          </div>
        </section>
        ...
        <section
          className={`mt-5 border p-5 ${paymobStatus.configured ? "border-emerald-300 bg-emerald-50" : "border-amber-300 bg-amber-50"}`}
        >
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[.14em] text-neutral-500">
                Paymob
              </p>
              ...
              <p className="mt-2 text-xs text-neutral-600">
                {paymobStatus.configured
                  ? ar
                    ? "كل الإعدادات موجودة. أكمل اختبار Paymob التجريبي قبل استقبال مدفوعات حقيقية."
                    : "All settings are present. Complete a Paymob sandbox acceptance test before taking live payments."
                  : ar
                    ? "لن يظهر Paymob للعملاء حتى تكتمل كل المفاتيح المطلوبة."
                    : "Paymob stays hidden from customers until every required setting is present."}
              </p>
            </div>
            {!paymobStatus.configured && (
              <div className="max-w-xl text-xs">
                <strong>{ar ? "الإعدادات الناقصة" : "Missing settings"}</strong>
                <p className="mt-1 font-mono text-[10px] leading-5">
                  {paymobStatus.missing.join(" · ")}
                </p>
              </div>
            )}
          </div>
          {webhookEvents.length > 0 && (
            <div className="mt-5 border-t border-black/10 pt-4">
              <p className="text-[10px] font-bold uppercase tracking-[.12em]">
                {ar ? "آخر إشعارات Paymob" : "Recent Paymob callbacks"}
              </p>
                ...
                {webhookEvents.map((event) => (
                  ...
                  >
                    <span className="font-mono">
                      {event.transaction_id ?? event.provider_event_id}
                    </span>
                    <span>
                      {event.outcome}
        ...
        <nav className="mt-8 flex flex-wrap gap-2">
          <Link
            ...
            className="border border-black/10 bg-white px-3 py-2 text-[10px] font-bold uppercase"
          >
            {ar ? "الكل" : "All"}
          </Link>
          {[
            ...
            <Link
              key={status}
              href={`/${locale}/admin/orders?status=${status}` as Route}
              className="border border-black/10 bg-white px-3 py-2 text-[10px] font-bold uppercase"
            >
              {status.replaceAll("_", " ")}
            </Link>
          ...
          {orders.map((order) => (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Tag value={order.payment_status} />
                  </div>
              ...
              <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_1fr_320px]">
                <section>
                  <h3 className="text-[10px] font-bold uppercase tracking-[.14em] text-neutral-500">
                    {ar ? "المنتجات" : "Items"}
                  </h3>
                ...
                <section className="text-xs leading-5">
                  <h3 className="text-[10px] font-bold uppercase tracking-[.14em] text-neutral-500">
                    {ar ? "التوصيل والدفع" : "Delivery & payment"}
                  </h3>
                  <address className="mt-3 not-italic">
                    ...
                    ,{" "}
                    {ar
                      ? (order.shipping_governorate_name_ar ??
                        order.governorate)
                      : (order.shipping_governorate_name_en ??
                        order.governorate)}
                  </address>
                  {order.shipping_zone_name_en && (
                    <p className="mt-2 text-neutral-500">
                      {ar
                        ? order.shipping_zone_name_ar
                        : order.shipping_zone_name_en}{" "}
                      · {formatMoney(order.shipping_minor, locale)} ·{" "}
                      {order.delivery_min_days}–{order.delivery_max_days}{" "}
                      {ar ? "أيام" : "days"}
                    </p>
                  )}
                  {order.discount_minor > 0 && (
                    <p className="mt-2 border-s-2 border-[#0e7468] ps-3 font-bold text-[#0e7468]">
                      {ar ? "كود الخصم" : "Discount code"}:{" "}
                      {order.discount_code} · −
                      {formatMoney(order.discount_minor, locale)}
                    </p>
                  )}
                  ...
                  {order.payment_method === "cod" && (
                    <div className="mt-2 border-s-2 border-[#0e7468] ps-3">
                      <strong>
                        {ar ? "المقدم" : "Deposit"}:{" "}
                        {formatMoney(order.cod_deposit_minor, locale)}
                      </strong>
                      <br />
                      {ar ? "المتبقي" : "Balance"}:{" "}
                      {formatMoney(order.cod_balance_due_minor, locale)}
                      <br />
                      {order.cod_deposit_method?.replaceAll("_", " ")}
                    </div>
                  )}
                  {order.customer_notes && (
                    <p className="mt-3 border-s-2 border-[#0e7468] ps-3">
                      {order.customer_notes}
                    </p>
                  )}
                  {order.payment_proofs.map((proof) => (
                    <div key={proof.id} className="mt-4">
                      <a
                        href={proofUrls.get(proof.id)}
                        target="_blank"
                        rel="noreferrer"
                        className="block"
                      >
                        <div className="relative aspect-video overflow-hidden bg-black/5">
                          {proofUrls.get(proof.id) && (
                            <Image
                              src={proofUrls.get(proof.id)!}
                              alt="Payment proof"
                              fill
                              sizes="360px"
                              unoptimized
                        ...
                        <span className="mt-2 block text-[10px] font-bold uppercase underline">
                          {ar ? "فتح إيصال الدفع" : "Open payment proof"} ·{" "}
                          {formatMoney(proof.amount_minor, locale)} ·{" "}
                          {proof.status}
                        </span>
                      </a>
                    </div>
                ...
                <form
                  key={`${order.id}-${order.status}-${order.payment_status}-${order.shipment_number ?? ""}`}
                  action={updateOrderAction}
                  className="grid content-start gap-3 border border-black/10 bg-[#f5f7f5] p-4"
                >
                    ...
                    <div className="grid gap-1">
                      ...
                      <div className="flex h-10 items-center justify-between border border-neutral-300 bg-neutral-200/60 px-2 text-xs font-semibold text-neutral-700">
                        ...
                        <span className="text-[10px] font-bold uppercase text-amber-800">
                          {ar ? "حالة نهائية (مغلق)" : "Terminal (Locked)"}
                        </span>
                      </div>
                      <span className="text-[10px] leading-3 text-neutral-500">
                        {ar
                          ? "تم تحرير المخزون تلقائياً. لا يمكن إعادة فتح الطلب الملغى."
                          : "Stock was released to inventory. This order cannot be reopened."}
                      </span>
                    </div>
                  ) : (
                    <label className="grid gap-1 text-[10px] font-bold uppercase">
                      {ar ? "حالة الطلب" : "Order status"}
                      <select
                        name="status"
                        defaultValue={order.status}
                        className="h-10 border bg-white px-2 text-xs"
                      >
                        {statuses.map((status) => (
                  ...
                  <label className="grid gap-1 text-[10px] font-bold uppercase">
                    {ar ? "قرار الإيصال" : "Proof decision"}
                    <select
                      name="proofStatus"
                      defaultValue=""
                      className="h-10 border bg-white px-2 text-xs"
                    >
                      ...
                      <option value="approved">
                        {ar
                          ? "موافقة — تأكيد الطلب تلقائياً"
                          : "Approve — confirm order automatically"}
                      </option>
                      <option value="rejected">
                        {ar
                          ? "رفض — إبقاء الطلب للمراجعة"
                          : "Reject — keep order in review"}
                      </option>
                    </select>
                    <span className="normal-case leading-4 text-neutral-500">
                      {ar
                        ? "قرار الإيصال يحدد حالة الدفع والطلب تلقائياً لمنع التعارض."
                        : "A proof decision sets the correct payment and order statuses automatically."}
                    </span>
                  </label>
                  <label className="grid gap-1 text-[10px] font-bold uppercase">
                    {ar ? "شركة الشحن" : "Courier"}
                    <input
                      name="courier"
                  ...
                  <label className="grid gap-1 text-[10px] font-bold uppercase">
                    {ar ? "رقم الشحنة" : "Shipment number"}
                    <input
                      name="shipmentNumber"
                      defaultValue={order.shipment_number ?? ""}
                      className="h-10 border bg-white px-2 text-xs"
                    />
                  </label>
                  <label className="grid gap-1 text-[10px] font-bold uppercase">
                    {ar ? "رابط التتبع" : "Tracking URL"}
                    <input
                      name="trackingUrl"
                      type="url"
                  ...
                  <label className="grid gap-1 text-[10px] font-bold uppercase">
                    {ar ? "ملاحظة للعميل" : "Customer update"}
                    <textarea
                      name="note"
                      maxLength={500}
                      className="min-h-16 border bg-white p-2 text-xs"
                    />
                  </label>
                  <button className="mt-1 flex h-11 items-center justify-center gap-2 bg-[#073b36] text-[10px] font-bold uppercase tracking-[.12em] text-white">
                    <Truck size={14} />
                    {ar ? "حفظ التحديث" : "Save update"}
                  </button>
                </form>
              </div>
            </article>
        ...
        {!orders.length && (
          <div className="mt-6 border border-dashed border-black/15 bg-white p-12 text-center text-sm text-neutral-500">
...
function Metric({
  ...
  return (
    <article
      className={`border p-5 ${alert ? "border-amber-300 bg-amber-50" : "border-black/10 bg-white"}`}
    >
      <Icon size={19} className="text-[#0e7468]" />
      <p className="mt-5 text-[10px] font-bold uppercase tracking-[.12em] text-neutral-500">
        {label}
      </p>
