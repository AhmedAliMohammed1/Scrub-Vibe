    ...
    <div className="mx-auto max-w-[1500px] px-5 py-8 md:px-10 md:py-12">
      ...
      <div className="mt-6 grid gap-5">{orders.map((order) => <article key={order.id} className="border border-black/10 bg-white p-5 md:p-7">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_1fr_320px]">
          ...
          <section className="text-xs leading-5"><h3 className="text-[10px] font-bold uppercase tracking-[.14em] text-neutral-500">{ar ? "التوصيل والدفع" : "Delivery & payment"}</h3><address className="mt-3 not-italic">{order.street_address}<br />{[order.building, order.floor, order.apartment].filter(Boolean).join(" · ")}<br />{ar ? (order.shipping_city_name_ar ?? order.city) : (order.shipping_city_name_en ?? order.city)}, {ar ? (order.shipping_governorate_name_ar ?? order.governorate) : (order.shipping_governorate_name_en ?? order.governorate)}</address>{order.shipping_zone_name_en && <p className="mt-2 text-neutral-500">{ar ? order.shipping_zone_name_ar : order.shipping_zone_name_en} · {formatMoney(order.shipping_minor, locale)} · {order.delivery_min_days}–{order.delivery_max_days} {ar ? "أيام" : "days"}</p>}<p className="mt-3 font-bold uppercase">{order.payment_method.replaceAll("_", " ")}</p>{order.payment_method === "cod" && <div className="mt-2 border-s-2 border-[#0e7468] ps-3"><strong>{ar ? "المقدم" : "Deposit"}: {formatMoney(order.cod_deposit_minor, locale)}</strong><br />{ar ? "المتبقي" : "Balance"}: {formatMoney(order.cod_balance_due_minor, locale)}<br />{order.cod_deposit_method?.replaceAll("_", " ")}</div>}{order.customer_notes && <p className="mt-3 border-s-2 border-[#0e7468] ps-3">{order.customer_notes}</p>}{order.payment_proofs.map((proof) => <div key={proof.id} className="mt-4"><a href={proofUrls.get(proof.id)} target="_blank" rel="noreferrer" className="block"><div className="relative aspect-video overflow-hidden bg-black/5">{proofUrls.get(proof.id) && <Image src={proofUrls.get(proof.id)!} alt="Payment proof" fill sizes="360px" unoptimized className="object-contain" />}</div><span className="mt-2 block text-[10px] font-bold uppercase underline">{ar ? "فتح إيصال الدفع" : "Open payment proof"} · {formatMoney(proof.amount_minor, locale)} · {proof.status}</span></a></div>)}</section>
