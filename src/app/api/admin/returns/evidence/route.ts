...
export async function GET(request: Request) {
  ...
  if (!path || path.includes("..")) return NextResponse.json({ error: "invalid_path" }, { status: 400 });
  const { data, error } = await createAdminClient().storage.from("return-evidence").createSignedUrl(path, 300);
