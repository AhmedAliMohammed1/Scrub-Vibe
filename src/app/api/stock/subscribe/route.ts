...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { message: "Enter a valid email and product selection." },
      { status: 400 },
    );
  }
  ...
  if (variantId) {
    ...
    if (!variant)
      return NextResponse.json(
        { message: "That colour or size is no longer available." },
        { status: 400 },
      );
  }
    ...
    return NextResponse.json(
      {
        message:
          locale === "ar"
            ...
            : "We could not save your alert. Please try again.",
      },
      { status: 500 },
    );
  }
  ...
  return NextResponse.json({
    message:
      locale === "ar"
        ? "تم حفظ التنبيه. سنراسلك عند توفر المنتج."
        : "Alert saved. We’ll email you when it is available.",
  });
}
