      ...
      {gaId && (
        <>
          ...
          <Script
            id="scrub-vibe-ga4"
            strategy="afterInteractive"
          >{`window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)}window.gtag=gtag;gtag('js',new Date());gtag('config','${gaId}',{send_page_view:false});`}</Script>
        </>
      )}
      {pixelId && (
        <Script
          id="scrub-vibe-meta-pixel"
          strategy="afterInteractive"
