  ...
  return (
    <section className="mt-14 border border-[#0e7468]/20 bg-[#f0f5f3] p-5 sm:p-7">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        {bundle.products.map((product, index) => (
          <Link
            ...
            className="flex items-center gap-3 bg-white p-3 transition hover:-translate-y-0.5 hover:shadow-sm"
          >
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-4 border-t border-[#0e7468]/15 pt-5">
        <strong>
          {ar ? "إجمالي المجموعة" : "Set total"}: {formatMoney(total, locale)}
        </strong>
        <button
          type="button"
          disabled={!available}
          onClick={addSet}
          className="inline-flex min-h-12 items-center justify-center gap-2 bg-[#073b36] px-6 text-xs font-bold uppercase tracking-[.12em] text-white disabled:cursor-not-allowed disabled:opacity-45"
        >
      ...
      <p className="sr-only" aria-live="polite">
        {added
          ? ar
            ? "تمت إضافة كل منتجات المجموعة إلى الحقيبة"
            : "All set items were added to your bag"
          : ""}
      </p>
    </section>
  );
