...
export async function processCommercialAutomation() {
  ...
  const [
    ...
    admin
      .from("inventory")
      .select(
        "variant_id, on_hand, reserved, low_stock_threshold, product_variants(product_id, sku)",
      ),
    admin
      .from("stock_subscriptions")
      .select("id, email, locale, product_id, variant_id")
      .eq("status", "active")
      .order("created_at")
      .limit(500),
    admin
      .from("inventory_alerts")
      .select("id, variant_id, last_notified_at")
      .eq("status", "open"),
  ]);
  if (inventoryError)
    throw new Error(`Inventory scan failed: ${inventoryError.message}`);
  if (subscriptionError)
    throw new Error(`Subscription scan failed: ${subscriptionError.message}`);
  ...
  const availableVariants = new Set(
    inventory.filter((row) => row.available > 0).map((row) => row.variant_id),
  );
  const availableProducts = new Set(
    inventory.filter((row) => row.available > 0).map((row) => row.productId),
  );
  ...
  for (const subscription of subscriptionRows ?? []) {
    const isAvailable = subscription.variant_id
      ? availableVariants.has(subscription.variant_id)
      : availableProducts.has(subscription.product_id);
    ...
    try {
      ...
      console.error(
        "[commercial/stock-notification] Email failed",
        subscription.id,
        error,
      );
    }
  ...
  const lowRows = inventory.filter(
    (row) => row.available <= row.low_stock_threshold,
  );
  const lowIds = new Set(lowRows.map((row) => row.variant_id));
  const openByVariant = new Map(
    (openAlerts ?? []).map((alert) => [alert.variant_id, alert]),
  );
  const newLowRows = lowRows.filter(
    (row) => !openByVariant.has(row.variant_id),
  );
  ...
  const resolvedIds = (openAlerts ?? [])
    .filter((alert) => !lowIds.has(alert.variant_id))
    .map((alert) => alert.id);
  ...
  if (newLowRows.length) {
    const lines = newLowRows
      .map(
        (row) =>
          `<li><strong>${row.sku}</strong>: ${row.available} available (threshold ${row.low_stock_threshold})</li>`,
      )
      .join("");
    await sendStaffEmail(
      `[Scrub Vibe] ${newLowRows.length} new low-stock alert${newLowRows.length === 1 ? "" : "s"}`,
