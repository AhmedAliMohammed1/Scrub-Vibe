      ...
      <div className="mt-10 grid gap-6 md:grid-cols-[1.3fr_.7fr]">
        ...
        <aside className="grid content-start gap-5">
          <section className="border border-black/10 bg-white p-5">
            ...
            <p className="mt-3 text-xs leading-5 text-neutral-600">{ar ? (order.shipping_city_name_ar ?? order.city) : (order.shipping_city_name_en ?? order.city)} · {ar ? (order.shipping_governorate_name_ar ?? order.governorate) : (order.shipping_governorate_name_en ?? order.governorate)}</p>
            {order.delivery_min_days && order.delivery_max_days && <p className="mt-2 text-xs font-semibold text-[#0e7468]">{ar ? `التوصيل المتوقع: ${order.delivery_min_days}–${order.delivery_max_days} أيام عمل` : `Estimated delivery: ${order.delivery_min_days}–${order.delivery_max_days} business days`}</p>}
          ...
          <section className="border border-black/10 bg-white p-5 text-sm">
            ...
            <dl className="mt-4 grid gap-2"><div className="flex justify-between"><dt>{ar ? "المنتجات" : "Items"}</dt><dd>{formatMoney(order.subtotal_minor, locale)}</dd></div>{order.discount_minor > 0 && <div className="flex justify-between font-bold text-[#0e7468]"><dt>{ar ? `خصم ${order.discount_code ?? ""}` : `${order.discount_code ?? "Promotion"} discount`}</dt><dd>−{formatMoney(order.discount_minor, locale)}</dd></div>}<div className="flex justify-between"><dt>{ar ? "الشحن الأساسي" : "Base shipping"}</dt><dd>{order.shipping_base_minor ? formatMoney(order.shipping_base_minor, locale) : (ar ? "مجاني" : "Free")}</dd></div>{order.shipping_discount_minor > 0 && <div className="flex justify-between text-[#0e7468]"><dt>{ar ? "خصم الشحن" : "Shipping discount"}</dt><dd>−{formatMoney(order.shipping_discount_minor, locale)}</dd></div>}{order.cod_surcharge_minor > 0 && <div className="flex justify-between"><dt>{ar ? "رسوم الدفع عند الاستلام" : "COD service fee"}</dt><dd>{formatMoney(order.cod_surcharge_minor, locale)}</dd></div>}{order.payment_method === "cod" && <><div className="flex justify-between font-bold text-[#0e7468]"><dt>{ar ? "المقدم" : "Deposit"}</dt><dd>{formatMoney(order.cod_deposit_minor, locale)}</dd></div><div className="flex justify-between"><dt>{ar ? "المتبقي عند الاستلام" : "Due on delivery"}</dt><dd>{formatMoney(order.cod_balance_due_minor, locale)}</dd></div></>}<div className="mt-2 flex justify-between border-t pt-3 font-bold"><dt>{ar ? "الإجمالي" : "Total"}</dt><dd>{formatMoney(order.total_minor, locale)}</dd></div></dl>
          </section>
        </aside>
      </div>
