...
- **Required inputs:** Supabase server secret (`SUPABASE_SECRET_KEY` or `SUPABASE_SERVICE_ROLE_KEY`); Twilio Verify credentials; Paymob public/secret/HMAC/integration credentials when onboarding completes; Vodafone Cash destination; InstaPay address; Resend key/sender domain.
- **Latest tests:** PASS — 12 files, 72 tests, including cart/wishlist sync and merge, shipping quotes, Paymob readiness/HMAC, payment-proof signatures, webhook idempotency invariants, COD deposit channels, OTP modes, authentication and catalogue behavior
...
- **Exact next action:** continue with transactional email/WhatsApp notifications or Paymob sandbox activation when credentials are provided.
