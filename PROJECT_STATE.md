...
- **Last successful task:** Fixed admin order updates to auto-refresh immediately without requiring manual reload (F5) using dynamic form remount keys, full router cache revalidation, and immediate navigation redirects preserving active filter parameters
- **Current branch:** `main`
...
- **Latest tests:** PASS — 13 files, 115 tests, including email templates (EN/AR), sender domain fallback, cart/wishlist sync, shipping quotes, Paymob readiness/HMAC, payment-proof signatures, webhook idempotency invariants, COD deposit channels, OTP modes, authentication and catalogue behavior
