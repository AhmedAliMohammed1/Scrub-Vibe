-- Migration: 20260908073000_idempotent_cart_sync.sql
-- Fix: Prevent checkout subtotal / cart quantity multiplication on tab switch or repeated syncs.
-- The previous UPSERT logic did `quantity = public.cart_items.quantity + excluded.quantity`,
-- which continuously multiplied cart quantities every time session refresh or tab switch occurred.
-- We update the UPSERT to be idempotent: using `greatest(public.cart_items.quantity, excluded.quantity)`
-- so that repeated synchronization never inflates cart quantities or totals.
...
begin
  ...
  -- 1. Ingest guest cart items (idempotent: greatest of existing and incoming, capped at 10)
  if p_cart is not null and jsonb_array_length(p_cart) > 0 then
    ...
    loop
      if v_item.variant_id > 0 and v_item.quantity > 0 then
        ...
        do update set
          quantity = least(greatest(greatest(public.cart_items.quantity, excluded.quantity), 1), 10),
          updated_at = timezone('utc'::text, now());
      end if;
    end loop;
  end if;

  -- 2. Ingest guest wishlist items (idempotent: on conflict do nothing)
