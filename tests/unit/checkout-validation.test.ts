...
describe("Egypt checkout validation", () => {
  it.each([
    ...
    "Nour O’Connor",
    "Álvaro Núñez",
  ])("accepts the reliable customer name %s", (name) => {
    expect(isReliableCustomerName(name)).toBe(true);
  });
  ...
  it.each([
    ...
    ". Mona",
    "A".repeat(121),
  ])("rejects the unreliable customer name %s", (name) => {
    expect(isReliableCustomerName(name)).toBe(false);
  });

  it("normalizes customer name spacing before checkout storage", () => {
    expect(normalizeCustomerName("  Mona   Ali  ")).toBe("Mona Ali");
    ...
    expect(result.success && result.data.customerName).toBe("Mona Ali");
    expect(
      result.success &&
        checkoutOrderSchema.safeParse({
          ...result.data,
          customerName: "Mona123 Ali",
        }).success,
    ).toBe(false);
  });
  ...
  it.each(["", "010123", "+201312345678", "+44123456789"])(
    "rejects %s",
    (input) => {
  ...
  it("accepts a complete order and rejects excessive quantities", () => {
    const order = {
      ...
      customerNotes: "",
      paymentMethod: "cod",
      codDepositMethod: "vodafone_cash",
    ...
    expect(
      checkoutOrderSchema.safeParse({
        ...order,
        items: [{ variantId: "42", quantity: 11 }],
      }).success,
    ).toBe(false);
  });
  ...
  it("requires a typed district when Other area is selected", () => {
    ...
    expect(checkoutOrderSchema.safeParse({ ...base, city: "" }).success).toBe(
      false,
    );
    expect(
      checkoutOrderSchema.safeParse({ ...base, city: "Daraw" }).success,
    ).toBe(true);
  });
  ...
  it("validates streetAddress boundaries in quick plain-text mode (5 to 300 chars)", () => {
    ...
    expect(
      checkoutOrderSchema.safeParse({ ...base, streetAddress: "15 St" })
        .success,
    ).toBe(true);
    // Too long (> 300 chars)
    expect(
